...
impl std::fmt::Display for SemanticError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SemanticError::DuplicateParam { name, span } =>
                write!(f, "{}:{}: duplicate parameter `{}`", span.line, span.col, name),
            SemanticError::AssignToUndefined { name, span } =>
                write!(f, "{}:{}: assignment to undefined variable `{}` (use `let`)", span.line, span.col, name),
            SemanticError::PrivateAccess { name, span } =>
                write!(f, "{}:{}: cannot access private name `{}` from outside its module", span.line, span.col, name),
            SemanticError::Custom { msg, span } =>
                write!(f, "{}:{}: {}", span.line, span.col, msg),
        }
    }
}
